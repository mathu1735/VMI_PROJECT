...
privileged aspect StockLocationController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> StockLocationController.listJson() {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        try {
            List<StockLocation> result = StockLocation.findAllStockLocations();
            return new ResponseEntity<String>(StockLocation.toJsonArray(result), headers, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<String>("{\"ERROR\":"+e.getMessage()+"\"}", headers, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<String> StockLocationController.createFromJson(@RequestBody String json, UriComponentsBuilder uriBuilder) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        try {
            StockLocation stockLocation = StockLocation.fromJsonToStockLocation(json);
            stockLocation.persist();
            RequestMapping a = (RequestMapping) getClass().getAnnotation(RequestMapping.class);
            headers.add("Location",uriBuilder.path(a.value()[0]+"/"+stockLocation.getId().toString()).build().toUriString());
    ...
    public ResponseEntity<String> StockLocationController.updateFromJson(@RequestBody String json, @PathVariable("id") Long id) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        try {
            StockLocation stockLocation = StockLocation.fromJsonToStockLocation(json);
